...
const ReadMore = (props) => {
    ...
    const truncatedString = children.substr(0, stringLength);
    const Wrapper = wrapper;

    return (
        <Wrapper className={classnames(className)} onClick={() => setStringLength(children.stringLength)}>
            <span>
                {truncatedString}
            </span>
            {
                truncatedString.length !== children.length ?
                    <span>
                        ...
                        <p className="c-blue mt-8 mb-0 lh-copy">
                         {labelText}
                        </p>
                    </span>
                    : null
