...
const ColorRow = ({number, palette}) => {
  return (
    <TRow className={`flex flex-middle bl bw-8 bc-${palette}-${number}`}>
      <TCol
        className="pl-8 flex-none flex flex-middle c-grey-50 fw-500 h-40"
        style={{ width: '132px' }}
      >
...
const ColorTable = (props) => {
  return (
    <Table>
      <TRow className="flex flex-between p-12 bg-tertiary-100 c-pointer">
        <TCol className="flex-none w-8 c-tertiary-500 fw-500">
          &nbsp;
        </TCol>
        <TCol className="pl-8 flex-1 c-tertiary-500 fw-500">
          SCSS variable
        </TCol>
        <TCol className="nml-8 flex-1 c-tertiary-500 fw-500">
          Text colour
        </TCol>
        <TCol className="pl-8 flex-1 c-tertiary-500 fw-500">
          Background colour
